...
UNTUK MENDAPATKAN API KEY ANDA BISA KUNJUNGI SITUS WEB INI: [green]https://licensi.yayandev.my.id/register[/]
JIKA TIDAK MENGERTI MENGGUNAKAN TOOLS SILAHKAN TONTON TUTORIAL DENGAN KETIK [green]tutorial[/]""",title="[green]PESAN[/]"))
        ...
        try:
            ...
            if get_arg["status"] == "error":
                print("");prints(Panel(f"😡[bold red] "+get_arg["msg"].replace("Anda telah menggunakan semua device yang tersisa, chat admin untuk menghapusnya", "Akses login di tolak! Dikarenakan anda sudah login di device atau perangkat sebelumnya.")));os.system("play-audio sound/tolak.mp3");exit()
            else:
...
silahkan gunakan dengan bijak yah tools nya 😊😊😊"""));time.sleep(2)
    ...
    def memek(self):
        asy.Logo().log();print("");prints(Panel("""😝[bold red] akses login di tolak dikarenakan anda sebelumnya sudah register[/]
